...
async function loadAnalytics() {
  try {
    // Load analytics data
    const [paymentsData, usersData, productsData] = await Promise.all([
      loadPaymentsData(),
      loadUsersData(),
      loadProductsData()
    ]);
    ...
    // Update quick stats
    updateAnalyticsStats(paymentsData, usersData, productsData);
    
    // Update charts
    updateAnalyticsCharts(paymentsData, usersData, productsData);
    
    // Update recent activity
    updateRecentActivity(paymentsData);
    
    // Update top products
    updateTopProducts(paymentsData);
...
function updateAnalyticsStats(paymentsData, usersData, productsData) {
  ...
  // Update DOM elements
  document.getElementById('adminTotalRevenue').textContent = `Rs ${totalRevenue.toLocaleString()}`;
...
function updateAnalyticsCharts(paymentsData, usersData, productsData) {
  ...
  updateAdminRevenueChart(paymentsData);
  
  // Update category chart
  updateAdminCategoryChart(paymentsData);
}
...
function initializeAdminCharts() {
  ...
  window.adminCharts.category = new Chart(categoryCtx, {
    type: 'doughnut',
    data: {
      labels: [],
      datasets: [{
...
function updateAdminRevenueChart(paymentsData) {
  const last30Days = getLast30DaysData(paymentsData);
  
  window.adminCharts.revenue.data.labels = last30Days.labels;
  window.adminCharts.revenue.data.datasets[0].data = last30Days.values;
  window.adminCharts.revenue.update();
}

function updateAdminCategoryChart(paymentsData) {
  const categoryData = calculateSalesByCategory(paymentsData);
  
  window.adminCharts.category.data.labels = categoryData.labels;
  window.adminCharts.category.data.datasets[0].data = categoryData.values;
  window.adminCharts.category.update();
}

function getLast30DaysData(paymentsData) {
...
function calculateSalesByCategory(paymentsData) {
  ...
  return {
    labels: sortedEntries.map(entry => formatCategoryName(entry[0])),
    values: sortedEntries.map(entry => entry[1])
  };
}

function updateRecentActivity(paymentsData) {
    ...
    return `
      <div class="activity-item">
        ...
        <div class="activity-content">
          <p>New order from ${payment.customerName || 'Guest'} - Rs ${(parseFloat(payment.totalAmount) || 0).toLocaleString()}</p>
...
function updateTopProducts(paymentsData) {
  const topProductsList = document.getElementById('adminTopProducts');
  ...
  topProductsList.innerHTML = sortedProducts.map(([productName, sales]) => `
...
function exportAnalyticsReport() {
  try {
    // This would export the current analytics data
